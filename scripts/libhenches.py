...
def parseClasses(classfilename):
  ...
  cfdesc.close()
  # strip comments
  classlist = list(filter(lambda s: not s.startswith('#'), classlist))
  ...
  for s in classlist:
    ...
    for score in tmp:
      ...
      minslist = list(map(int, minslist))
      if len(minslist) == 1:
        # mmm...  curry
...
def parseNames(namefile):
  ...
  names = list(filter(lambda s: not s.startswith('#'), names))
  out = {}
  i = names.index("")
  out['M'] = names[0:i]
  out['F'] = names[i+1:]
...
def genHenches(numHenches, level, classes, market, al, spells, names, profs,gear,notabs=True):
  ...
  for k in range(0,numHenches):
    ...
    if profs != []:
      ...
      for i in range(0, math.floor(bonusprofs)):
        done = False
        # 50% chance that we try to increase a prof we already have
        if random.random() < 0.5:
          profshave = list(profmap.keys())
